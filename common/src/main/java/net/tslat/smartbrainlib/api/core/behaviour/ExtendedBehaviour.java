...
public abstract class ExtendedBehaviour<E extends LivingEntity> extends Behavior<E> {
	...
	/// things when the entity ends this task.
	...
	/// Set the length that the task should run for, once activated.
	...
	/// Set the length that the task should run for once activated, randomly selected between two values.
	/// The value used is in _ticks_.
	///
	...
	/// dynamically predicating behaviours.
	...
	/// [ExtendedBehaviour#getMemoryRequirements()] have already been checked.
	///
	...
	/// be overridden by other abstract subclasses.
	/// If overriding, ensure you either call `super` or manually call
	/// `stop(E)` yourself.
	///
	/// @param level    The level the entity is in
	...
	/// needed or perform the required actions.
	/// By this stage any memory requirements set in
	/// [ExtendedBehaviour#getMemoryRequirements()] are true, so any memories
	/// paired with [MemoryStatus#VALUE_PRESENT] are safe to retrieve.
	///
	...
	public boolean tryExpire(ServerLevel level, E entity, long gameTime) {
	...
	/// method should only be overridden by other abstract subclasses.
	/// If overriding, ensure you either call super or manually call the
	/// [ExtendedBehaviour#stopCondition] check yourself.
	///
	/// @param level    The level the entity is in
	/// @param entity   The entity the brain belongs to
	/// @param gameTime The current gameTime (in ticks) of the level
	...
	/// for behaviours so that all required auto-handling is safely contained without
	/// super calls.
	/// This is called when the behaviour is ticked. Be aware this is called _every
	/// tick_, so use tick reduction if needed to minimise performance impacts of
	/// goals.
	/// NOTE: Memory requirements are _not_ guaranteed at this stage. If you are
	...
	///
	/// @return The [List] of [Memories][MemoryModuleType] and their
	///         associated required [status][MemoryStatus]
