...
public class SBLSkeleton extends Skeleton implements SmartBrainOwner<SBLSkeleton> {
	...
	public List<ExtendedSensor<SBLSkeleton>> getSensors() {
		return ObjectArrayList.of(new NearbyPlayersSensor<>(), 															// Keep track of nearby players
				new NearbyLivingEntitySensor<SBLSkeleton>().setPredicate((target, entity) -> target instanceof Player	// Keep track of nearby entities the Skeleton is interested in
						|| target instanceof IronGolem || target instanceof Wolf || (target instanceof Turtle turtle && turtle.isBaby() && !turtle.isInWater())));
	}

	@Override
	public BrainActivityGroup<SBLSkeleton> getCoreTasks() {
		return BrainActivityGroup.coreTasks(new AvoidSun<>(), 					// Keep pathfinder avoiding the sun
	...
	public BrainActivityGroup<SBLSkeleton> getIdleTasks() {
		return BrainActivityGroup.idleTasks(new FirstApplicableBehaviour<SBLSkeleton>(	// Run only one of the below behaviours, trying each one in
																						// order. Include explicit generic typing because javac is silly
	...
	public BrainActivityGroup<SBLSkeleton> getFightTasks() {
		return BrainActivityGroup.fightTasks(
				new StopAttackingIfTargetInvalid<>(
						target -> !target.isAlive() || target instanceof Player && ((Player) target).isCreative()), // Invalidate the attack target if
				...
				new FirstApplicableBehaviour<>( 															// Run only one of the below behaviours, trying each one in order
						...
						new AnimatableMeleeAttack<>(0).whenStarting(entity -> setAggressive(true))	// Melee attack
